...
function render(items) {
  if (!items.length) {
    grid.innerHTML = '<div class="empty-state"><strong>아직 게시된 글이 없습니다.</strong><span>부자형의 새로운 문장을 준비하고 있습니다.</span></div>';
    return;
  }
  grid.innerHTML = items.map(post => `
    <article class="post-card" tabindex="0" data-id="${post.id}">
      <span class="post-card-no">THREAD SEODANG · ${String(post.source_no || '').padStart(3, '0')}</span>
...
function openPost(id) {
  ...
  if (!post) return;
  document.querySelector('#reader-no').textContent = `스레드 서당 ${post.source_no ? `#${post.source_no}` : ''}`;
...
async function loadPosts() {
  if (!configured()) {
    notice.innerHTML = '<p class="notice">스레드 서당 데이터 연결을 준비하고 있습니다.</p>';
    render([]);
    return;
  }
  const client = window.supabase.createClient(config.url, config.anonKey);
  const { data, error } = await client.from('posts').select('id, source_no, title, body, published_at').eq('is_published', true).order('published_at', { ascending: false });
  if (error) {
    notice.innerHTML = `<p class="notice error">글을 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.</p>`;
    render([]);
    return;
  }
  posts = data || [];
...
grid.addEventListener('keydown', event => {
  const card = event.target.closest('.post-card');
  if (card && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); openPost(card.dataset.id); }
});
