...
pickAdminList.addEventListener('click', async event => {
  ...
  if (button.dataset.action === 'reparse-pick') {
    try {
      const result = await api('reparse-auction-pick', { id: item.id }, 90000);
      await loadAuctionPicks();
      const added = result && result.images ? ` 사진 ${result.images}장도 가져왔습니다.` : '';
      message(pickStatus, `스레드 원문에서 본문을 다시 읽었습니다.${added} 손으로 채운 소재지·입찰일·사건번호는 그대로입니다.`);
    } catch (error) { message(adminMessage, error.message, true); }
  }
